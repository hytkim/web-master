...
const MakeRow = (item) => {
  // 카드 전체를 감싸는 div
  //'ITEMS_PRICE',
  const fields = ['ITEMS_NAME','DISCOUNT_PERCENT','ITEMS_CATEGORY','HISTORY_ITEM_IMAGE'];
  console.log('log: ',item.items_category);
  ...
  // 최종 가격 계산 (가격 * (1 - 할인율))
  const finalPriceValue = Math.round(item.ITEMS_PRICE * (1 - item.DISCOUNT_PERCENT));//Math.floor(item.ITEMS_PRICE * (1 - item.DISCOUNT_PERCENT));
...
//작동 원리 분석용 코드
// const MakeRow_Me = (item) => {
//   // 리턴받을 div.card 생성
//   let div = document.createElement('div');
//   div.setAttribute('class', 'card');

//   // div.card.children[0] => img
//   let img = document.createElement('img');
//   img.setAttribute('src', item.HISTORY_ITEM_IMAGE);
//   img.setAttribute('alt', item.ITEMS_NAME);
  
//   // div.card.children[1] => cardcontent
//   let cardContent = document.createElement('div');
//   cardContent.setAttribute('class', cardContent);
  
//   // div.cardcontent.children[0] => p.name
//   let p = document.createElement('p');
//   p.setAttribute('class','name');
//   p.innerHTML = item.ITEMS_NAME;

//   // div.cardcontent.children[1] => div.price-section
//   let priceSection = document.createElement('div');
//   priceSection.setAttribute('class','price-section');

//   let discontRate = document.createElement('span');
//   // discountRate.setAttribute();
//   console.log('할인율뭔데',item.DISCOUNT_PERCENT);
//   discontRate.innerHTML = `${item.DISCOUNT_PERCENT * 100}%`;;

//   let finalPrice = document.createElement('span');
//   finalPrice.setAttribute('class','final-price');
  
//   finalPrice.innerHTML = !item.DISCOUNT_PERCENT 
//   ? `<span><ruby>${ item.ITEMS_PRICE}<rt></rt></ruby></span>`
//   :`<span><ruby>${ Math.round(item.ITEMS_PRICE * item.DISCOUNT_PERCENT)}<rt>${item.ITEMS_PRICE}</rt></ruby></span>`;

//   priceSection.appendChild(discontRate);
